...
static inline real pe::transformToAxis(
    const Box& box,
    const Vector3D& axis
) {
    return
...
static inline real pe::penetrationOnAxis(
    const Box& one,
    const Box& two,
    const Vector3D& axis,
...
static inline bool pe::overlapOnAxis(
    const Box& one,
    const Box& two,
    const Vector3D& axis,
    const Vector3D& toCentre
...
static inline Vector3D pe::contactPoint(
    const Vector3D& pOne,
    const Vector3D& dOne,
    real oneSize,
    const Vector3D& pTwo,
...
unsigned int pe::sphereAndSphere(
    const Ball& one,
    const Ball& two,
