...
const std::string anisotropicFragmentShader = R"(
    ...
    void main(){
        ...
        for (int i = 0; i < numActiveLights; ++i) {
            ...
            // Calculate tangent space reflection vector
            vec3 R = reflect(-lightDir, Normal);
            vec3 viewDirection = normalize(viewPos - FragPos);
            vec3 H = normalize(lightDir + viewDirection);
            vec3 T = normalize(reflect(-lightDir, Tangent));
            float specular = pow(max(dot(H, T), 0.0), shininess); // Anisotropic term
            vec3 specularColor = lightColors[i].rgb * specular * lightColors[i].a;
            finalSpecular += specularColor;
        }

        // Combines diffuse and specular with some ambient lighting
